...
# Template Workshop
[![License](https://img.shields.io/github/license/akatief/test-worshop-template)](https://opensource.org/licenses/Apache-2.0)
[![Python Versions](https://img.shields.io/badge/Python-3.9-blue.svg?style=flat&logo=python&logoColor=white)](https://www.python.org/)
[![CI](https://github.com/akatief/test-worshop-template/actions/workflows/main.yml/badge.svg)](https://github.com/akatief/test-worshop-template/actions/workflows/main.yml)
...
```bash
test_worshop_template n # Will return fib(n)
```
...
- `test_worshop_template/__init__.py`: Defines the content of the test_worshop_template package. It's important to configure it properly to have a cleaner `import` structure in your code. Use it to avoid ugly [absolute imports](https://www.geeksforgeeks.org/absolute-and-relative-imports-in-python/).
- `test_worshop_template/cli.py`: Defines how to handle CLI arguments and calls the Fibonacci class.
- `test_worshop_template/fibonacci.py`: Contains the Fibonacci class.
...
```py
from test_worshop_template import BaseClass
...
```bash
pytest -v --cov-fail-under=90 --cov=test_worshop_template -l --tb=short --maxfail=1 tests/
```

Let's now turn to the class you've just implemented. You want to make sure your code *actually* works and handles all cases. For example, have you thought of what would happen if someone called `test_worshop_template -1`? First, write code in Fibonacci to handle what to do with negative (or zero) numbers. Then, write tests for `Fibonacci` to check that your code actually works. Your test should all complete successfully and coverage (as computed by the command above) should be at least 90%.
...
- `test_worshop_template/fibonacci.py`: Contains the Fibonacci class. Change it to fix bugs found during testing.
...
```
GitHub Actions are called after a specific trigger is detected, in the case above a push action on the main branch of your repo. GitHub Actions are implemented in the `.github/workflows` folder, and results of previous runs are in your repository's [Actions tab](https://github.com/akatief/test-worshop-template/actions). You can inspect the action output to understand why a specific action fails. 

Familiarize with the [Actions interface](https://github.com/akatief/test-worshop-template/actions) and understand why some of them fail. Then, apply changes to the repository to fix them. Finally, push the changes to GitHub to run the actions again. They should all show a ✅ on the page.
